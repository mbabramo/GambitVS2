...
private:
  ...
  void  div(const std::vector<V>& denom, TrieNode<V>* n, int current, int pivot);
   ...
   __declspec(noinline) void deleteNodes(TrieNode<V> *n){
	if (!n) return;
  ...
  /*
   __declspec(noinline) void remove (TrieNode<V> *n){
	if (!n) return;
	size_type i, s=n->children.size();
	for (i=0;i<s;++i) if (n->children[i]){
	  remove(n->children[i]);
  ...
  */
  ...
  //in order traversal
  template <class UnaryFunction>
   __declspec(noinline) void in_order_subtree(UnaryFunction& f, TrieNode<V> *n, bool debug=false)
  {
    if (n==NULL) return;
    size_type i,s=n->children.size();
    bool is_leaf=true;
