...
NashLcpBehaviorSolver<T>::Solve(const BehaviorSupportProfile &p_support) const
{
  ...
  int ntot;
  solution.ns1 = p_support.NumSequences(1); // num actions for player 1 across all information sets
  ...
  Matrix<T> A(1,ntot,0,ntot); // matrix ntot X ntot + 1 (beginning number at row 1 and column 0, with 
  Vector<T> b(1,ntot); // vector with ntot entries

  solution.maxpay = p_support.GetGame()->GetMaxPayoff() + Rational(1); // maximum payoff + 1
