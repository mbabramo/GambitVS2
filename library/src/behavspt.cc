...
; n++)
	is_infosets_node_active.Append(false);
      is_players_node_active.Append(is_infosets_node_active); // so the outer list is info sets, the inner list is nodes (members of the info set), and all are initialized to false.
    }
    m_infosetActive[pl] = is_players_infoset_active; // information sets active only
    m_nonterminalActive[pl] = is_players_node_active; // whether node is active
  }
