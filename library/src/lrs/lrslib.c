...
lrs_read_dat (lrs_dat * Q, int argc, char *argv[])
{
  ...
  if (infile > 0)			/* command line argument overides stdin   */
    {
	  errno_t er = fopen_s(&lrs_ifp, argv[infile], "r");
      if (lrs_ifp == NULL)
	{
	  printf ("\nBad input file name\n");
	  return (FALSE);
  ...
  if ((!Q->nash && argc == 3) || (Q->nash && argc == 4)) 
    {
	  errno_t er = fopen_s(&lrs_ofp, argv[argc - 1], "r");
	  if (lrs_ofp == NULL)
	{
...
/* process input file */
  if( fscanf_s (lrs_ifp, "%s", name, sizeof(name)) == EOF)
	    {
	      fprintf (lrs_ofp, "\nNo begin line");
	      return (FALSE);
	    }
  ...
  while (strcmp (name, "begin") != 0)	/*skip until "begin" found processing options */
	...
	{
	  if (fscanf_s(lrs_ifp, "%ld", &dec_digits) == EOF)
	    {
	      fprintf (lrs_ofp, "\nNo begin line");
	      return (FALSE);
	    }
      ...
      else if (firstline)
	{
      ...
      if (fscanf_s (lrs_ifp, "%s", name) == EOF)
	{
	  fprintf (lrs_ofp, "\nNo begin line");
	  return (FALSE);
	}
  ...
  if (fscanf_s (lrs_ifp, "%ld %ld %s", &Q->m, &Q->n, name, sizeof(name)) == EOF)
    {
      fprintf (lrs_ofp, "\nNo data in file");
      return (FALSE);
    }
...
/* read in flags */
  while (fscanf_s (lrs_ifp, "%s", name, sizeof(name)) != EOF)
    {
      if (strncmp (name, "*", 1) == 0)	/* skip any line beginning with * */
	{
	  c = name[0];
      ...
      if (strcmp (name, "checkpoint") == 0)
	{
	  ...
	  if(fscanf_s (lrs_ifp, "%ld", &seconds) == EOF)
            {
	...
	{
          Q->etrace =0;
	  if(fscanf_s (lrs_ifp, "%ld %ld", &Q->strace, &Q->etrace)==EOF)
             Q->strace =0;
      ...
      if (strcmp (name, "restart") == 0)
	{
	  Q->restart = TRUE;
          if(Q->voronoi)
           {
             if(fscanf_s (lrs_ifp, "%ld %ld %ld %ld", &Q->count[1], &Q->count[0], &Q->count[2], &P->depth)==EOF)
               return FALSE;
             fprintf (lrs_ofp, "\n*%s V#%ld R#%ld B#%ld h=%ld data points", name, Q->count[1], Q->count[0], Q->count[2], P->depth);
            }
          else if(hull)
            {
	    if( fscanf_s (lrs_ifp, "%ld %ld %ld", &Q->count[0], &Q->count[2], &P->depth)==EOF)
...
/* Used also with lponly flag                                          */
	...
	{
	  if(fscanf_s (lrs_ifp, "%ld", &Q->frequency)==EOF)
/*2010.7.7  set default to zero = print only when outputting vertex/ray/facet */
	...
	{
	  if(fscanf_s (lrs_ifp, "%ld", &dict_limit)==EOF)
              dict_limit=1;
	  fprintf (lrs_ofp, "\n*cache %ld", dict_limit);
	  if (dict_limit < 1)
	    dict_limit = 1;
	...
	{
	  if(fscanf_s (lrs_ifp, "%ld", &Q->maxdepth)==EOF)
                    Q->maxdepth=MAXD;
	...
	{
	  if(fscanf_s (lrs_ifp, "%ld", &Q->maxoutput)==EOF)
             Q->maxoutput = 100;
	...
	{
	  if(fscanf_s (lrs_ifp, "%ld", &Q->maxcobases)==EOF)
             Q->maxcobases = 1000;
	...
	{
	if( fscanf_s (lrs_ifp, "%ld", &Q->mindepth)==EOF)
           Q->mindepth = 0;
	...
	{
	  if(fscanf_s (lrs_ifp, "%ld", &Q->seed)==EOF)
               Q->seed = 3142;
	  fprintf (lrs_ofp, "\n*seed= %ld ", Q->seed);
	}
	...
	{
	  if(fscanf_s (lrs_ifp, "%ld", &Q->runs)==EOF)
             Q->runs=1;
	  fprintf (lrs_ofp, "\n*%ld %s", Q->runs, name);
	}
...
// 2015.2.9   Estimates will continue until estimate is less than subtree size
      if (strcmp (name, "subtreesize") == 0)
        {
          if(fscanf_s (lrs_ifp, "%ld", &Q->subtreesize)==EOF)
             Q->subtreesize=MAXD;
 ...
 /* returns true if denominator is not one        */
{
  ...
  if(fscanf_s (lrs_ifp, "%s", in, sizeof(in)) == EOF)
     return(FALSE);
...
readlinearity (lrs_dat * Q)	/* read in and check linearity list */
{
  long i, j;
  long nlinearity;
  if(fscanf_s (lrs_ifp, "%ld", &nlinearity)==EOF )
    {
...
/* assign local variables to structures */
  ...
  for (j = Q->nlinearity; j < d; j++)	/* note we place these after the linearity indices */
    {
      if(fscanf_s (lrs_ifp, "%ld", &facet[j])==EOF)
        {
