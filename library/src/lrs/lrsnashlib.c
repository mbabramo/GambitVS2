  ...
  for (s = 0; s < g->nstrats[ROW]; s++) {
    for (t = 0; t < g->nstrats[COL]; t++) {
                        if(g->payoff[s][t][ROW].den == 1)
                                sprintf_s(out[ROW], MAXINPUT, "%ld,", g->payoff[s][t][ROW].num);
                        else
                                sprintf_s(out[ROW], MAXINPUT, "%ld/%ld,", g->payoff[s][t][ROW].num, g->payoff[s][t][ROW].den);
                        if(g->payoff[s][t][COL].den == 1)
                                sprintf_s(out[COL], MAXINPUT, "%ld", g->payoff[s][t][COL].num);
                        else
                                sprintf_s(out[COL], MAXINPUT, "%ld/%ld", g->payoff[s][t][COL].num, g->payoff[s][t][COL].den);
